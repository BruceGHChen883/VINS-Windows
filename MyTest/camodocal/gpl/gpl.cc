...
int
clock_gettime(int X, struct timeval *tp)
{
    LARGE_INTEGER           t;
    FILETIME            f;
    double                  microseconds;
...
int
clock_gettime(int X, struct timespec *tp) //这里是timespec
{
	LARGE_INTEGER           t;
	FILETIME            f;
	double                  microseconds;
...
unsigned long long timeInMicroseconds(void)
{
	struct timespec tp;  //这里是timespec
#ifdef __APPLE__
	tp = orwl_gettime();
#else
	clock_gettime(CLOCK_REALTIME, &tp);
...
long int
timestampDiff(uint64 t1, uint64 t2)
{
    if (t2 > t1)
    {
        uint64 d = t2 - t1;
